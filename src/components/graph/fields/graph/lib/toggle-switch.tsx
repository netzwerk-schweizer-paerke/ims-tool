...
type Props = {
  /** Accessible name. Required when no visible `label` is rendered — a bare role="switch" has none. */
  ariaLabel?: string
  // Optional because the field's JSON value is undefined until its init effect lands —
  // the input must stay controlled across that transition, or it keeps its mount-time state.
  checked?: boolean
  disabled?: boolean
  label?: string
...
export const ToggleSwitch = ({ ariaLabel, checked, disabled = false, label, onChange }: Props) => {
  const id = useId()
  return (
    <div>
      <input
