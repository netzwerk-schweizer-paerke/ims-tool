...
export const GraphTextArea = ({ className = '', onTextChange, readOnly = false, value }: Props) => {
  const locale = useLocale()

  return (
    <textarea
      className={`graph-text-area graph-hyphenate resize-none rounded-2xl text-center leading-snug read-only:cursor-default focus:outline-none ${className}`}
